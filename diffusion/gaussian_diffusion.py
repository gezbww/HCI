...
class GaussianDiffusion:
    ...
    def ddim_sample_loop_progressive(
        ...
        indices = list(range(0, self.num_timesteps, self.num_timesteps // num_steps))[::-1]
        # indices = list(range(self.num_timesteps - skip_timesteps))[::-1]
    ...
    def plms_sample(
        ...
        if order > 1 and old_out is None:
            ...
            eps_2, _, _ = get_model_output(mean_pred, t - time_interval)
    ...
    def plms_sample_loop_progressive(
        ...
        time_interval = self.num_timesteps // num_steps
        indices = list(range(0, self.num_timesteps, time_interval))[::-1]
        # indices = list(range(self.num_timesteps - skip_timesteps))[::-1]
    ...
    def dpm_solver_pp_sample(
        ...
        """
        二阶DPM-Solver++采样函数（纯二阶版本）
        
        参数说明：
        - model: 扩散模型
        - x: 当前样本 x_t
        - t: 当前时间步t
        - time_interval: 时间步间隔（通常为1）
        - old_out: 历史信息缓存，格式为 {"old_x0": [], "old_t": []}
        """
        # 1. 获取模型输出的辅助函数
        def get_model_output(x, t):
            with th.set_grad_enabled(cond_fn_with_grad and cond_fn is not None):
        ...
        # 2. 提取alpha和sigma
        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, x.shape)
        alpha = th.sqrt(alpha_bar)
        sigma = th.sqrt(1 - alpha_bar)
        
        # 3. DPM-Solver++核心：使用数据预测模式
        # 获取模型输出（预测x0）
        _, out, _ = get_model_output(x, t)
        x0_pred = out["pred_xstart"]  # 这是关键：数据预测
        
        # 4. 计算lambda (logSNR)
        # lambda_t = log(alpha_t / sigma_t) = log(sqrt(alpha_bar / (1-alpha_bar)))
        lambda_t = th.log(alpha / sigma)
        
        # 5. 计算前一个时间步的lambda
        t_prev = t - time_interval
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod, t_prev, x.shape)
        alpha_prev = th.sqrt(alpha_bar_prev)
        sigma_prev = th.sqrt(1 - alpha_bar_prev)
        lambda_prev = th.log(alpha_prev / sigma_prev)
        
        # 6. 二阶DPM-Solver++更新
        if old_out is None or len(old_out.get("old_x0", [])) < 1:
            # 6.1 首次执行：需要中间步骤启动二阶方法
            
            # 计算中间时间步（t - time_interval/2）
            t_mid = t - time_interval % 2
            
            # 获取中间点的alpha和sigma
            alpha_bar_mid = _extract_into_tensor(self.alphas_cumprod, t_mid, x.shape)
            alpha_mid = th.sqrt(alpha_bar_mid)
            sigma_mid = th.sqrt(1 - alpha_bar_mid)
            lambda_mid = th.log(alpha_mid / sigma_mid)
            
            # 第一阶段：用一阶方法预测中间点
            # 公式: x_{t-1/2} = (α_{t-1/2}/α_t) * x_t - σ_{t-1/2} * (e^{h/2} - 1) * ε_θ
            # 其中 h = λ_{t-1} - λ_t，但这里我们使用 h/2 = λ_{t-1/2} - λ_t
            
            # 先获取当前噪声预测（用于一阶更新）
            eps, _, _ = get_model_output(x, t)
            
            # 计算一阶更新到中间点
            h_half = lambda_mid - lambda_t
            exp_h_half = th.exp(h_half)
            
            # 一阶更新公式（DDIM-like，但用lambda表示）
            x_mid = (alpha_mid / alpha) * x - sigma_mid * (exp_h_half - 1) * eps
            
            # 获取中间点的模型输出（关键：预测x0）
            _, out_mid, _ = get_model_output(x_mid, t_mid)
            x0_pred_mid = out_mid["pred_xstart"]
            
            # 6.2 二阶更新：使用中间点的预测
            h = lambda_prev - lambda_t
            exp_h = th.exp(h)
            
            # DPM-Solver++二阶更新公式（数据预测模式）：
            # x_{t-1} = (α_{t-1}/α_t) * x_t - σ_{t-1} * (e^h - 1) * ε_θ^{(1)}
            #          - (σ_{t-1}/2) * (e^h - 1 - h) * (ε_θ^{(2)} - ε_θ^{(1)})
            
            # 将x0预测转换为噪声预测（用于高阶修正项）
            eps = (x - alpha * x0_pred) / sigma  # 当前点的噪声预测
            eps_mid = (x_mid - alpha_mid * x0_pred_mid) / sigma_mid  # 中间点的噪声预测
            
            # 计算修正项
            correction = (sigma_prev / 2) * (exp_h - 1 - h) * (eps_mid - eps)
            
            # 二阶更新
            sample = (alpha_prev / alpha) * x - sigma_prev * (exp_h - 1) * eps - correction
            
            # 保存历史信息
            old_info = {
                "old_x0": [x0_pred, x0_pred_mid],
                "old_t": [t, t_mid],
                "old_lambda": [lambda_t, lambda_mid]
            }
            
        else:
            # 6.3 后续步骤：使用历史信息进行二阶多步更新
            old_x0 = old_out["old_x0"]
            old_t = old_out["old_t"]
            old_lambda = old_out["old_lambda"]
            
            # 保存当前信息
            old_x0.append(x0_pred)
            old_t.append(t)
            old_lambda.append(lambda_t)
            ...
            if len(old_x0) >= 2:
                # 线性外推预测前一个时间步的x0
                # 使用lambda作为自变量（logSNR空间）
                x0_prev1, x0_prev2 = old_x0[-2], old_x0[-1]  # x0_pred at t-1 and t
                lambda_prev1, lambda_prev2 = old_lambda[-2], old_lambda[-1]
                
                # 线性外推系数
                r = (lambda_prev - lambda_prev2) / (lambda_prev2 - lambda_prev1)
                
                # 外推预测x0
                x0_pred_extrapolated = x0_prev2 + r * (x0_prev2 - x0_prev1)
                
                # 使用外推的x0进行更新
                eps_extrapolated = (x - alpha * x0_pred_extrapolated) / sigma
                
                # 一阶更新公式（使用外推的噪声）
                h = lambda_prev - lambda_t
                exp_h = th.exp(h)
                sample = (alpha_prev / alpha) * x - sigma_prev * (exp_h - 1) * eps_extrapolated
                ...
                h = lambda_prev - lambda_t
                exp_h = th.exp(h)
                sample = (alpha_prev / alpha) * x - sigma_prev * (exp_h - 1) * eps
        ...
        # 7. 边界条件处理（t=0时直接返回x0）
        nonzero_mask = (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
        sample = sample * nonzero_mask + x0_pred * (1 - nonzero_mask)
    ...
    def dpm_solver_pp_sample_loop(
        ...
        """
        final = None
        for sample in self.dpm_solver_pp_sample_loop_progressive(
            model,
            shape,
            num_steps=num_steps,
            noise=noise,
    ...
    def dpm_solver_pp_sample_loop_progressive(
        self,
        model,
        shape,
        num_steps=500,
