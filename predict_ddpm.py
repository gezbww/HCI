...
#     model.load_state_dict(torch.load('pretrained/{}.pth'.format(args.model_name), map_location='cuda'))
...
#     processor = Wav2Vec2Processor.from_pretrained("pretrained/hubert-xlarge-ls960-ft",local_files_only=True)
...
def test_model(args):
    ...
    state_dict = torch.load(ckpt_path, map_location='cpu')  # 在 CPU 上加载
