...
def _compare_versions(op, got_ver, want_ver, requirement, pkg, hint):
    ...
    # 检查特定包的特殊情况
    # 添加这行代码来跳过 tokenizers 的版本检查
    if pkg == 'tokenizers':
        # 如果安装的 tokenizers 版本 >= 0.10.1，就认为满足要求
        if version.parse(got_ver) >= version.parse("0.10.1"):
            return  # 直接返回，不进行后续检查
        # 否则继续执行原有的检查逻辑
    ...
    if not ops[op](version.parse(got_ver), version.parse(want_ver)):
        raise ImportError(
            f"{requirement} is required for a normal functioning of this module, "
            f"but found {pkg}=={got_ver}.{hint}"
        )
