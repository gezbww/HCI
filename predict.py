...
def test_model(args):
    ...
    # ================== 2. 显存友好地加载权重 ==================
    ckpt_path = os.path.join('pretrained_models', f'{args.model_name}.pth')
    print(f"[INFO] loading checkpoint from {ckpt_path} to CPU ...")
    ...
    # ================== 3. 设备与精度策略 ==================
    # low_mem = True 时，模型和采样全部在 CPU 上跑，几乎不占 GPU 显存
    if args.low_mem:
        model_device = torch.device('cpu')
        diff_device = torch.device('cpu')
        use_half = False
        print("[INFO] low_mem 模式启用：模型与 diffusion 采样全部在 CPU 上运行。")
    else:
        model_device = torch.device(args.device)
        diff_device = model_device
        use_half = (model_device.type == 'cuda' and torch.cuda.is_available())
        if use_half:
            print(f"[INFO] 使用 GPU({args.device}) + FP16 推理。")
        else:
            print(f"[INFO] 在设备 {model_device} 上用 FP32 推理。")
    ...
    # ================== 4. 读取模板 ==================
    template_file = os.path.join(args.data_path, args.dataset, args.template_path)
    print(f"[INFO] loading templates from {template_file}")
    with open(template_file, 'rb') as fin:
        templates = pickle.load(fin, encoding='latin1')
    ...
    speech_array, sampling_rate = librosa.load(os.path.join(wav_path), sr=16000)
    print("[INFO] loading HuBERT processor on CPU ...")
    processor = Wav2Vec2Processor.from_pretrained("facebook/hubert-xlarge-ls960-ft")
    ...
    num_frames = int(audio_feature.shape[0] / sampling_rate * args.output_fps)
    num_frames -= 1
    print(f"[INFO] num_frames = {num_frames}, vertice_dim = {args.vertice_dim}")
        ...
        if use_half and diff_device.type == "cuda":
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                prediction = diffusion.p_sample_loop(
                    ...
                    noise=None,
                    const_noise=False,
                    device=str(diff_device)  # 不再写死 "cuda"
                )
        else:
            # CPU 或 GPU FP32 路径
    ...
    out_file_name = test_name + "_" + args.dataset + "_" + args.subject + "_condition_" + args.condition
    np.save(os.path.join(args.result_path, out_file_name), prediction)
    print(f"[INFO] prediction saved to {os.path.join(args.result_path, out_file_name + '.npy')}")
...
def main():
    ...
    parser.add_argument("--low_mem", action="store_true",
                        help="极限省显存：把模型+diffusion 采样全部放在 CPU 上运行。")

    # 新增：不渲染开关（只推理，不跑 pyrender）
    parser.add_argument("--no_render", action="store_true",
                        help="只做预测，不进行渲染（服务器 / 无显示环境建议开启）。")
    ...
    # the blendshape results are to be rendered in external engines
    # like Maya, Blender, UE
    if (not args.no_render) and args.dataset in ["BIWI", "multiface", "vocaset"]:
        render(args)
