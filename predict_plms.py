...
#         # 默认用半精度；如果你需要严格 FP32，可以把这里设成 False
#         use_half = True
...
#     # 原代码的 reshape 逻辑保持不变
#     audio_feature = np.reshape(audio_feature, (-1, audio_feature.shape[0]))  # (T, 1) 或类似
#     audio_feature = torch.tensor(audio_feature, dtype=torch.float16)
#     if use_half:
#         audio_feature = audio_feature.half()
#     audio_feature = audio_feature.to(device=device)
...
#     # ================== 核心推理（显存优化：no_grad + half + device 可配置） ==================
#     with torch.no_grad():
...
def test_model(args):
    ...
    # ================== 设备策略 ==================
    # model_device: 模型所在设备（非 vocaset/非 low_mem 可以用 GPU）
    # diff_device: diffusion 采样所在设备（vocaset+low_mem 强制 CPU）
    ...
    # diffusion 对象本身是一些 tensor / numpy，放在哪无所谓，这里按 diff_device 处理
    diffusion = create_gaussian_diffusion(args)
    ...
    # ================== 核心推理 ==================
    # 注意：这里完全保持原始 plms_sample_loop 的逻辑，只是换了设备 / 精度
    with torch.no_grad():
        if use_half and diff_device.type == "cuda":
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                prediction = diffusion.plms_sample_loop(
                    model.to(diff_device),                     # 确保 model 在 diff_device
                    (1, num_frames, args.vertice_dim),
                    num_steps=args.num_steps,
                    clip_denoised=False,
                    model_kwargs={
...
def main():
    ...
    "--no_render",
    action="store_true",
    help="只做预测，不做 pyrender 渲染（在无显示的服务器上用这个）"
)
    # 新增：极限省显存开关
    parser.add_argument("--low_mem", action="store_true",
                        help="vocaset 时启用：把 diffusion 采样全部放在 CPU 上，避免 GPU OOM。")
